...
add_image_size('homepage-thumb', 160, 160, true);
add_image_size('single-thumb', 600, 300, true);
...
*		return new WP_Error( 'rest_not_logged_in', __( 'You are not current logged in.' ), array( 'status' => 401 ) );
*	}
*	return $result;
*});
**/
if (! class_exists('Aod_Walker_Nav_Menu')) {
  class Aod_Walker_Nav_Menu extends Walker_Nav_Menu {
	/**
    ...
    // Mod para visualizar el menu con bootstrap. Adicional detecta si esta enlace de github y coloca el icono
      // clases para a tag
    if (strpos($atts['href'],'github')) {
      $atts['class'] = 'nav-link active d-inline-flex align-items-center';
